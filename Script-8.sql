...
-- 교수의 소속 학과에 따라 보너스 지급, 101학과일 경우 급여의 10%, 102학과는 20% 201학과는 30% 
-- 나머지는 0%
SELECT 
	PROFNO, 
	NAME, 
...
-- 101학과 학생들의 몸무게 평균과 합계를 구하라 도한 최대 키와 최소 키도 구하라
SELECT COUNT(*), AVG(WEIGHT), SUM(WEIGHT), MAX(HEIGHT), MIN(HEIGHT)
FROM STUDENT
WHERE DEPTNO = 101;

-- 그룹바이에서 명시해야 셀렉트에 칼럼이 들어갈수있다
...
-- 학생의 학과와 학년을 기준으로 그룹하여 학과와 학년별 인원수와 평균 체중을 구하고 반올림 처리하여라 시험문제?
...
-- 학생수가 4명 이상인 학년에대해 학생 평균 신장과 체중을 조회
SELECT GRADE, COUNT(*)
FROM STUDENT
WHERE DEPTNO = 101
GROUP BY GRADE
...
-- 이 순서 FROM > WHERE > GROUP > SELECT > ORDER
