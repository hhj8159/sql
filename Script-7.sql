...
SELECT DEPTNO , GRADE ,STUDNO , NAME 
FROM STUDENT s 
WHERE 
	DEPTNO = 102
	AND GRADE IN(1, 4);
...
-- 학생 테이블에서 학년 내림차순으로 정렬 / 이름, 학년, 전화번호 조회
SELECT NAME, GRADE, TEL 
FROM STUDENT s 
ORDER BY GRADE DESC ; 

-- 학생 테이블에서 학년 내림차순으로 정렬, 같은 학년일 경우 이름으로 정렬
-- / 이름, 학년, 전화번호 조회
...
-- 1학년 학생들의 생년월일, 태어날 달은 조회(주민번호를 통해서)
...
-- 부서 테이블에서 부서 이름의 마지막 '과' 글자를 제거하라
...
SELECT ROUND(MONTHS_BETWEEN(SYSDATE, '93/12/27')) A FROM DUAL;
